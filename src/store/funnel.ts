...
interface FunnelState {
  ...
  /**
   * Номер прохода. Растёт при каждом сбросе и НИКОГДА не сохраняется.
   *
   * ЗАЧЕМ. Сброс возвращает шаг в начало, но если человек УЖЕ на первом шаге,
   * значение шага не меняется — React не видит причины перемонтировать экран, и
   * сцена остаётся ровно там, где стояла: такты проиграны, кнопка на месте.
   * Со стороны это выглядит как сломанная кнопка «начать сначала».
   *
   * Номер прохода входит в ключ экрана, поэтому сброс перемонтирует его всегда,
   * даже когда шаг совпал. Это единственное, ради чего поле существует; на
   * содержание воронки оно не влияет.
   */
  runId: number;
...
export const useFunnel = create<FunnelState>()(
  persist(
    (set, get) => ({
      ...
      /**
       * Полный сброс прохода.
       *
       * Номер прохода НЕ обнуляется вместе с остальным, а увеличивается: иначе
       * сброс из начала воронки не менял бы ни одного значения в сторе и экран
       * не перемонтировался бы (см. комментарий к `runId`).
       */
      reset: () => set((s) => ({ ...initial, runId: s.runId + 1 })),
    }),
    {
