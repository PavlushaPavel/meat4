 ...
 */
function figureOf(cue: PreframeCue): string | undefined {
  /**
   * НА ТАКТАХ ОБВИНЕНИЯ АВТОР УХОДИТ ИЗ КАДРА.
   *
   * Не ради композиции, а по смыслу: «лиды плохие — смотрим Директ», «пора
   * менять директолога» — это приговор безличный и тотальный, в нём нет
   * собеседника. Экран на эти такты принадлежит ему одному.
   *
   * Заодно это чинит настоящую поломку: приговор живёт на заднике, за фигурой,
   * и голова автора разрезала надпись пополам — «…НОВАТ …ИРЕКТ». Двигать плашку
   * было некуда: справа от фигуры остаётся сто точек, а выше начинается рама.
   */
  if (cue === 'blame') return undefined;
 ...
 *
 * ЧТО ЗДЕСЬ БЫЛО И ПОЧЕМУ ЭТО НЕ РАБОТАЛО ДВАЖДЫ.
 *
 * Сначала стояла табличка с именем района и три красных указателя, втыкающихся
 * в неё по одному на такт. Указатели читались как схема — «сюда смотри», — а в
 * записи происходит другое: одно и то же обвинение повторяют, и каждый раз оно
 * весит больше.
 *
 * Потом плашку стали УВЕЛИЧИВАТЬ ТРАНСФОРМАЦИЕЙ, и это оказалось хуже схемы.
 * `scale` растягивает всё подряд: вместе с буквами раздувались отступы поля и
 * полосы ленты, пропорции плашки уплывали, а к последнему повтору текст уезжал
 * за оба края экрана. На мини-аппе, где экран 360 точек и обрезать нечего, это
 * просто брак.
 *
 * ЗДЕСЬ РАСТЁТ КЕГЛЬ, А НЕ КАРТИНКА. Плашка перекладывается вокруг текста:
 * отступы поля и толщина ленты остаются постоянными в настоящих пикселях, длинная
 * строка переносится на две, ширина ограничена полями экрана. Обвинение всё
 * равно занимает всё больше места — но остаётся плашкой, а не растянутым
 * изображением плашки.
 *
 * УДАР НА КАЖДОМ ПОВТОРЕ. Смена кегля — это раскладка, её нельзя анимировать
 * плавно, не заплатив кадрами. Поэтому новый размер приходит новым узлом и
 * оседает короткой пружиной: читается как «навалилось», а не как «плавно
 * поехало».
 */
function BlameScene({ beat }: { beat: number }) {
  const reduceMotion = useReducedMotion();
  const step = Math.min(beat, BLAME_TYPE.length - 1);

  return (
    <div
      aria-hidden="true"
      /*
        Поля по краям обязательны: это телефон, и упереться в кромку экрана
        плашка не имеет права. Верхняя треть — за головой автора, туда взгляд
        уходит сам, когда читает заголовок ниже.
      */
      className="pointer-events-none absolute inset-x-5 top-[20%] flex justify-center"
    >
      <motion.div
        key={step}
        ...
        className="w-full max-w-[22rem] rotate-[-1.5deg]"
      >
        <Plate className="w-full">
          <span
            style={{ fontSize: BLAME_TYPE[step] }}
            className="block text-balance text-center font-display font-semibold uppercase leading-[0.94] tracking-tight"
          >
            {preframe.blame}
          </span>
        </Plate>
...
/**
 * Кегль приговора по повторам, в пикселях.
 *
 * Шаг замедляется к концу: привыкание работает и здесь, и чтобы давление
 * ощущалось ровным, приращение должно уменьшаться. Потолок в 56 подобран под
 * узкий экран — на 360 точках при полях по 20 «ВИНОВАТ ДИРЕКТ» встаёт в две
 * строки и держится внутри плашки, не упираясь в её поле.
 */
const BLAME_TYPE = [22, 32, 42, 50, 56] as const;
