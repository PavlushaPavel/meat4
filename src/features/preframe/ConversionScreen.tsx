...
function ChainScene() {
  ...
  return (
    ...
    <div className="overflow-hidden">
      ...
      >
        {/*
          Ужиманием занимается сама коробка кадра (`StageBox` в `SceneShell`):
          она измеряет содержимое и подбирает масштаб под доступную высоту.
          Здесь стояли ручной `scale-[0.88]` и отрицательный отступ — обходные
          приёмы против прежней коробки фиксированной высоты. Вместе с новой
          подгонкой они ужимали связку дважды, и она обрезалась ровно на
          ПРОДАЖЕ, то есть на том участке, ради которого схема и показывается.
        */}
        <ConversionChain step="conversion" dense />
      </motion.div>
    </div>
  );
}
